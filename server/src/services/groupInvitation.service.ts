...
class GroupInvitationService {
  async sendInvitation(userId: number, groupId: number, inviteeId: number, message?: string) {
    ...
    const inviterIsMember = await groupRepository.checkMembership(groupId, userId);
    ...
    const isMember = await groupRepository.checkMembership(groupId, inviteeId);
  ...
  async acceptInvitation(userId: number, invitationId: number) {
    ...
    // Add member
    await groupRepository.addMember(inv.idGroup.idGroup, userId);
    // Delete invitation
    await groupInvitationRepository.deleteInvitationById(invitationId);
